...
const crear_nueva_fila = (boleto, clientes) => {
    const cliente = clientes.find(c => c.id === boleto.id_cliente);
    const fila = document.createElement('tr');
    const contenido = `
        <td class="td" data-td>${boleto.nro_asiento}</td>
        <td>${boleto.id_programacion}</td>
        <td>${cliente ? cliente.nombre : '-'} ${cliente ? cliente.apellido : ''}</td>
        <td>
...
const actualizarTabla = async (boletos) => {
    while (table.firstChild) {
        table.removeChild(table.firstChild);
    }
    const clientes = await clientService.lista_clientes();
    boletos.forEach(boleto => {
        const nuevaFila = crear_nueva_fila(boleto, clientes);
